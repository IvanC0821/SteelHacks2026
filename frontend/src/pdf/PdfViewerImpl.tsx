...
/** What one `blob` opened into. Held as a single keyed record so a new blob reads as "still
 *  loading" during render instead of being reset from inside the effect. */
interface Opened {
  blob: Blob;
...
export function PdfViewer({
  ...
  const [settled, setSettled] = useState<Opened | null>(null);
  const [available, setAvailable] = useState(0);

  // derived during render: a record for another blob is stale, which reads as "loading"
  const current = settled && settled.blob === blob ? settled : null;
  ...
  useEffect(() => {
    ...
    openDocument(blob)
      .then(async (document) => {
        ...
        if (!live) return;
        setSettled({ blob, doc: document, sizes: next, error: null });
      })
      .catch(() => {
        if (live) {
          setSettled({ blob, doc: null, sizes: NO_SIZES, error: "This PDF could not be opened." });
        }
      });
    return () => {
      live = false;
