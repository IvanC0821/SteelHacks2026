...
export function PageThumbnails({
  ...
  // one keyed record, so a new blob reads as "still loading" during render rather than being
  // reset from inside the effect
  const [settled, setSettled] = useState<{ blob: Blob; doc: PdfDocument; count: number } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    openDocument(blob)
      .then((document) => {
        ...
        opened = document;
        setSettled({ blob, doc: document, count: document.numPages });
      })
      .catch(() => {
